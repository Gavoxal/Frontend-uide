...
const Grades = () => {
    ...
    const fetchGrades = async () => {
        setLoading(true);
        try {
            const data = await TutorService.getAssignedStudentsGrades();
            setStudents(data);
        } catch (error) {
    ...
    const handleSave = async (studentId, weekIndex) => {
        ...
        try {
            await ActivityService.gradeEvidencia(evidenceId, {
                calificacion: gradeValue,
                feedback: 'Calificación actualizada desde el panel de notas'
            });

            // Actualizar estado local
            const newStudents = [...students];
            const studentIdx = newStudents.findIndex(s => s.studentId === studentId);
            const updatedStudent = { ...newStudents[studentIdx] };
            ...
            updatedStudent.promedio = validGrades.length > 0 ? sum / validGrades.length : 0;

            newStudents[studentIdx] = updatedStudent;
    ...
    const getGradeColor = (grade) => {
        ...
        // En este sistema, solo hay aprobado >= 7 o reprobado
        return 'error.main';
    };
    ...
    return (
        <Box sx={{ p: 3 }}>
            <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Box>
                    ...
                    <Typography variant="body1" color="text.secondary">
                        Registro semanal de avances y promedio final (Aprobación: &gt;= 7.0)
                    </Typography>
                </Box>
                <TextField
                        ...
                        <Table stickyHeader size="small">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell align="center" sx={{ fontWeight: 'bold', backgroundColor: '#e3f2fd', minWidth: 100, zIndex: 10 }}>
                                        Promedio
                                    </TableCell>
                                </TableRow>
                            </TableHead>
                                ...
                                {filteredStudents.length > 0 ? (
                                    filteredStudents.map((row) => (
                                        <TableRow key={row.studentId} hover>
                                            ...
                                            <TableCell align="center" sx={{ backgroundColor: '#f5f9ff' }}>
                                                <Chip
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={18} align="center" sx={{ py: 8 }}>
                                            <Typography color="text.secondary">
                                                {loading ? "Cargando datos..." : "No se encontraron estudiantes asignados"}
                                            </Typography>
                                        </TableCell>
                                    </TableRow>
                                )}
            ...
            <Box sx={{ mt: 3, display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                ...
                <Box sx={{ ml: 'auto' }}>
                    <Typography variant="caption" color="text.secondary">
                        * Solo se pueden editar las notas de trabajos ya entregados.
                    </Typography>
                </Box>
            </Box>
